...
func TestOpenAIChatModel_Invoke(t *testing.T) {
	...
	t.Logf("OpenAI Invoke reply: %q", resp.Content)
}
...
func TestOpenAIChatModel_Stream(t *testing.T) {
	...
	t.Logf("OpenAI Stream received %d chunks", chunks)
}

func TestOpenAI_CreateAgent_ToolLoop(t *testing.T) {
	model := newOpenAIModel(t)
	...
	t.Logf("OpenAI agent final reply: %q", reply[len(reply)-1].Content)
}

func TestOpenAI_StreamEvents(t *testing.T) {
	model := newOpenAIModel(t)
	...
	t.Logf("OpenAI StreamEvents received %d text deltas", deltas)
}
