...
type StreamChunk struct {
    Namespace string     // "" = root graph; subgraph path segments joined by "/" (matches checkpoint NS)
    Mode      StreamMode
...
- **S6. Metadata.Step alignment (M2 debt).** `UpdateState` checkpoints save with `Step: base.Step+1`; new-turn input checkpoints continue the step counter (not hardcoded -1 — only a thread's FIRST input checkpoint is -1). Existing tests asserting the old numbering are updated.
...
- Produces: S1/S2/S3 API. Debug payloads: `task` = `{"id","name","input","triggers"}` (triggers = the edge/Command that scheduled the task where known, else the node name); `task_result` = `{"id","name","error","result","interrupts"}`; `checkpoint` = `{"config","parent_config","values","metadata","next"}` (Go's Checkpoint has no per-task `tasks` detail — document the omission).
- Emission timing (from Reference Semantics): `values` after input batch + after each superstep commit + merged `__interrupt__` on pause; `updates` per task as outcomes are collected (post-`wg.Wait`, in deterministic task order — document the divergence from Python's as-they-finish timing, needed because Go applies writes only after all tasks complete); replayed writes on resume re-emitted as `updates`; `debug` task events at dispatch, task_result at completion, checkpoint after each save.

- [ ] **Step 1: Write failing tests** — linear 3-superstep graph: `values` sequence (input + per superstep); `updates` per-task chunks with exact `{node: {key: value}}` payloads; `debug` task/task_result/checkpoint sequence with step numbers; multi-mode combination yields all modes interleaved chronologically; pause run yields the `__interrupt__` chunks in both `values` and `updates`; resume re-emits replayed `updates`; `Subgraphs: false` drops subgraph chunks, `true` includes them with correct `Namespace`; early break cancels (no goroutine leak — assert via `runtime.NumGoroutine` before/after or a done-channel probe).
...
- Produces: `sqlite.New(path string, serde checkpoint.Serializer) (*Saver, error)` implementing `checkpoint.Saver`; `:memory:` supported. Schema exactly per Reference Semantics (two tables, WAL, type+value columns, metadata as plain JSON). Reserved-channel writes use `idx` -1/-2/-3 with INSERT OR REPLACE; regular writes INSERT OR IGNORE with `idx` = position (mirrors Python's WRITES_IDX_MAP: `__error__`→-1, `__tasks__`→-2, `__interrupt__`→-3). Checkpoint blob = serde-typed serialization of a JSON-safe projection of `Checkpoint` (ChannelValues per-value through the serde; versions maps plain); document the exact projection in code.
...
- [ ] **Step 2: Write failing tests** — mirror the Python sqlite test categories relevant to a checkpoint saver: put/get-latest/get-by-id round-trip (channel values with messages + interrupts survive serde), List newest-first + Before + Limit, PutWrites visibility incl. reserved-channel replace-vs-ignore semantics, DeleteThread, D3 parent links, concurrent access (WAL), `:memory:`.
- [ ] **Step 3: Implement `sqlite.go`. Step 4: `go test ./...` in the nested module PASS; root module gate still PASS (root does not import the nested module).**
