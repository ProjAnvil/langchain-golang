...
func TestShellSessionPersistsCwdAndEnv(t *testing.T) {
	...
	r, err := s.Execute(t.Context(), "pwd", 10*time.Second)
	if err != nil {
	...
	if _, err := s.Execute(t.Context(), "cd /tmp && pwd", 10*time.Second); err != nil {
	...
	if _, err := s.Execute(t.Context(), "export FOO=bar", 10*time.Second); err != nil {
		t.Fatalf("Execute export: %v", err)
	}
	r, err = s.Execute(t.Context(), "echo $FOO", 10*time.Second)
	if err != nil {
...
func TestShellSessionExitCode(t *testing.T) {
	...
	r, err := s.Execute(t.Context(), "false", 10*time.Second)
	if err != nil {
...
func TestShellSessionCapturesStderr(t *testing.T) {
	...
	r, err := s.Execute(t.Context(), "echo to-stderr >&2", 10*time.Second)
	if err != nil {
...
func TestShellSessionNotStarted(t *testing.T) {
	...
	if _, err := s.Execute(t.Context(), "pwd", time.Second); err == nil {
...
func TestShellSessionTimeoutKillsAndRestarts(t *testing.T) {
	...
	r, err := s.Execute(t.Context(), "sleep 30", 200*time.Millisecond)
	if err != nil {
	...
	r, err = s.Execute(t.Context(), "echo recovered", 10*time.Second)
	if err != nil {
...
func TestShellSessionCtxCancelKillsAndRestarts(t *testing.T) {
	...
	r, err = s.Execute(t.Context(), "echo after-cancel", 10*time.Second)
	if err != nil {
