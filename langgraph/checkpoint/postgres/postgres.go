...
// SCHEDULED counterpart, so the -2 slot stays empty. Python's map has NO
// TASKS entry — its TASKS writes use the positional idx — and neither does
// Go's: the executor persists Command.Goto routing as __tasks__ writes, and
// they take the positional idx like any regular channel, so a
// multi-destination Goto's several __tasks__ writes in one batch each keep
// their own slot and ALL survive (Python and MemorySaver parity; an earlier
// revision mapped __tasks__ to -2, collapsing same-batch __tasks__ writes
// to one row). RESUME at -4 is lossless: the executor persists the consumed
// resume prefix as ONE write carrying the whole ordered list (Python parity,
